    ...
    if (injectDll)
    {
        ...
        if (remoteImageBase != nullptr)
        {
            ...
            if (StartHooking(hProcess, dllMemory, (DWORD_PTR)remoteImageBase))
            {
                ...
                {
                    wprintf(L"Failed to write hook dll data %d\n",GetLastError());
                }
            }
        }
    }
...
void ReadSettings()
{
    ...
    g_hdd.EnableCreateProcessAHook = g_settings.opts().hookCreateProcessA;
    g_hdd.EnableCreateProcessWHook = g_settings.opts().hookCreateProcessW;
