...
t_CreateProcessA _CreateProcessA = 0;
t_CreateProcessW _CreateProcessW = 0;
...
bool ApplyKernel32Hook(HOOK_DLL_DATA * hdd, HANDLE hProcess, BYTE * dllMemory, DWORD_PTR imageBase)
{
    ...
    void * HookedCreateProcessA = (void *)(GetDllFunctionAddressRVA(dllMemory, "HookedCreateProcessA") + imageBase);
    void * HookedCreateProcessW = (void *)(GetDllFunctionAddressRVA(dllMemory, "HookedCreateProcessW") + imageBase);
    ...
    _CreateProcessA = (t_CreateProcessA)GetProcAddress(hCurrent, "CreateProcessA");
    _CreateProcessW = (t_CreateProcessW)GetProcAddress(hCurrent, "CreateProcessW");


    g_log.LogDebug(L"ApplyKernel32Hook -> _GetTickCount %p _GetTickCount64 %p _GetLocalTime %p _GetSystemTime %p _OutputDebugStringA %p _CreateProcessA %p _CreateProcessW %p",
        _GetTickCount,
        _GetTickCount64,
        _GetLocalTime,
        _GetSystemTime,
    ...
    if (hdd->EnableCreateProcessAHook == TRUE)
    {
        g_log.LogDebug(L"ApplyKernel32Hook -> Hooking CreateProcessA");
        HOOK(CreateProcessA);
    }
    if (hdd->EnableCreateProcessWHook == TRUE)
    {
        g_log.LogDebug(L"ApplyKernel32Hook -> Hooking CreateProcessW");
        HOOK(CreateProcessW);
    }
...
void RestoreKernel32Hooks(HOOK_DLL_DATA * hdd, HANDLE hProcess)
{
    ...
    RESTORE_JMP(CreateProcessA);
    RESTORE_JMP(CreateProcessW);
    ...
    FREE_HOOK(CreateProcessA);
    FREE_HOOK(CreateProcessW);
