    ...
    else
    {
        ...
        if (injectDll)
        {
            ...
            if (remoteImageBase)
            {
                ...
                {
                    g_log.LogError(L"Failed to write hook dll data %d\n", GetLastError());
                }
            }
            else
            {
...
void FillHookDllData(HANDLE hProcess, HOOK_DLL_DATA *hdd)
{
    ...
    hdd->EnableCreateProcessAHook = g_settings.opts().hookCreateProcessA;
    hdd->EnableCreateProcessWHook = g_settings.opts().hookCreateProcessW;
