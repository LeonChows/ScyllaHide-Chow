...
#ifdef X64DBG
	CheckDlgButton(hWnd, IDC_CREATEPROCESSA, opts->hookCreateProcessA);
	CheckDlgButton(hWnd, IDC_CREATEPROCESSW, opts->hookCreateProcessW);
#endif
...
#ifdef X64DBG
    opts->hookCreateProcessA = (IsDlgButtonChecked(hWnd, IDC_CREATEPROCESSA) == BST_CHECKED);
    opts->hookCreateProcessW = (IsDlgButtonChecked(hWnd, IDC_CREATEPROCESSW) == BST_CHECKED);
#endif
...
INT_PTR CALLBACK OptionsDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_SAVEPROFILE:
        {
            ...
            auto success = ask_str(&szNewProfileName,0, "New profile name?");
            if (!success)
                break;
