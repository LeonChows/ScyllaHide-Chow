...
typedef LSTATUS(WINAPI* pRegOpenKeyExA_t)(HKEY, LPCSTR, DWORD, REGSAM, PHKEY);
typedef LSTATUS(WINAPI* pRegCloseKey_t)(HKEY);

typedef void* (__cdecl* pMalloc_t)(size_t);
typedef void(__cdecl* pFree_t)(void*);
typedef void* (WINAPI* pmemset_t)(void*, int, size_t);

typedef int  (WINAPI* pWsprintfA_t)(LPSTR, LPCSTR, ...);
typedef int  (WINAPI* pWsprintfW_t)(LPWSTR, LPCWSTR, ...);
typedef int  (WINAPI* pMessageBoxA_t)(HWND, LPCSTR, LPCSTR, UINT);
typedef int (WINAPI* pMultiByteToWideChar_t)(UINT, DWORD, LPCSTR, int, LPWSTR, int);


// 函数指针
pRegQueryValueExA_t N_pRegQueryValueExA_t;
pRegOpenKeyExA_t N_pRegOpenKeyExA_t;
pRegCloseKey_t N_pRegCloseKey_t;
pMalloc_t       N_malloc;
pFree_t         N_free;
pmemset_t         N_memset;
pWsprintfA_t    N_wsprintfA;
pWsprintfW_t    N_wsprintfW;
pMessageBoxA_t  N_MessageBoxA;
pMultiByteToWideChar_t  N_MultiByteToWideChar;

HMODULE hModuleKer = nullptr;
HMODULE hcrt = nullptr;
HMODULE hUser32 = nullptr;
#define SUB_KEY "exefile\\shell\\Debug with x64dbg\\Command"
...
HMODULE myGetmoudlehandle(LPCSTR Mname) {
	return  LI_FN(LoadLibraryA)(Mname);
}

DWORD64 myGetProcAddress(HMODULE hModule, LPCSTR ProcName) {
	return (DWORD64)LI_FN(GetProcAddress)(hModule, ProcName);
}

// 初始化函数指针
void HoookFuncinit() {

	if (hModuleKer == nullptr || hcrt == nullptr || hUser32 == nullptr)
	{
		hModuleKer = myGetmoudlehandle("kernel32.dll");
		hcrt = myGetmoudlehandle("ucrtbase.dll");
		if (hcrt)
			hcrt = myGetmoudlehandle("msvcrt.dll");
		hUser32 = myGetmoudlehandle("user32.dll");
	}
	if (N_wsprintfW == nullptr || N_pRegQueryValueExA_t == nullptr || N_pRegOpenKeyExA_t == nullptr || N_pRegCloseKey_t == nullptr || N_malloc == nullptr || N_free == nullptr || N_wsprintfA == nullptr || N_MessageBoxA == nullptr || N_MultiByteToWideChar == nullptr)
	{
		N_pRegQueryValueExA_t = (pRegQueryValueExA_t)myGetProcAddress(hModuleKer, "RegQueryValueExA");
		N_pRegOpenKeyExA_t = (pRegOpenKeyExA_t)myGetProcAddress(hModuleKer, "RegOpenKeyExA");
		N_pRegCloseKey_t = (pRegCloseKey_t)myGetProcAddress(hModuleKer, "RegCloseKey");
		N_malloc = (pMalloc_t)myGetProcAddress(hcrt, "malloc");
		N_free = (pFree_t)myGetProcAddress(hcrt, "free");
		N_memset = (pmemset_t)myGetProcAddress(hcrt, "memset");
		N_wsprintfA = (pWsprintfA_t)myGetProcAddress(hUser32, "wsprintfA");
		N_wsprintfW = (pWsprintfW_t)myGetProcAddress(hUser32, "wsprintfW");
		N_MessageBoxA = (pMessageBoxA_t)myGetProcAddress(hUser32, "MessageBoxA");
		N_MultiByteToWideChar = (pMultiByteToWideChar_t)myGetProcAddress(hModuleKer, "MultiByteToWideChar");
	}
}

// 从注册表读取字符串值
LPSTR GetRegDefaultValueString(LPCSTR szSubKey)
{
	HKEY hKey;
	if (N_pRegOpenKeyExA_t(HKEY_CLASSES_ROOT, szSubKey, 0, KEY_READ, &hKey) != ERROR_SUCCESS)
		return NULL;
	...
	// 获取大小（默认值传 NULL）
	if (N_pRegQueryValueExA_t(hKey, NULL, NULL, &dwType, NULL, &dwSize) != ERROR_SUCCESS)
	{
		N_pRegCloseKey_t(hKey);
		return NULL;
	}
	...
	LPSTR szValue = (LPSTR)N_malloc(dwSize + 1);
	if (!szValue)
	{
		N_pRegCloseKey_t(hKey);
		return NULL;
	...
	N_memset(szValue, 0, dwSize + 1);

	if (N_pRegQueryValueExA_t(hKey, NULL, NULL, &dwType, (LPBYTE)szValue, &dwSize) != ERROR_SUCCESS)
	{
		N_free(szValue);
		N_pRegCloseKey_t(hKey);
		return NULL;
	...
	N_pRegCloseKey_t(hKey);
	return szValue;  // 调用者 free
}

/* 1) 最简单、通用、可读性最好 */
size_t Mystelen(const char* s)
{
	if (!s) return 0;                /* 约定：NULL 返回 0 */
	const char* p = s;
	while (*p) ++p;                  /* 遇到 '\0' 结束 */
	return (size_t)(p - s);
}
...
// 从右边删除 len 个字符
LPSTR StrRightTrim(LPSTR szStr, int len)
{
	int strLen = Mystelen(szStr);
	if (len <= 0 || len > strLen)
		return szStr;   // 不删除或越界时原样返回

	szStr[strLen - len] = '\0';  // 从右边截断
	return szStr;
}

// 从左边删除 len 个字符
LPSTR StrLeftTrim(LPSTR szStr, int len)
{
	int strLen = Mystelen(szStr);
	if (len <= 0 || len > strLen)
		return szStr;   // 不删除或越界时原样返回
...
HANDLE WINAPI HookedCreateProcessA(LPCSTR lpApplicationName, LPSTR lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, DWORD dwCreationFlags, LPVOID lpEnvironment, LPCSTR lpCurrentDirectory, LPSTARTUPINFOA lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation)
{
	if (HookDllData.dCreateProcessA)
	{
		HoookFuncinit();
		LPCSTR subKey = SUB_KEY;
		LPSTR _start_path = GetRegDefaultValueString(subKey);
		_start_path = StrRightTrim(_start_path, 6);
		_start_path = StrLeftTrim(_start_path, 1);

		char buffer[1024] = { 0 };
        N_wsprintfA(buffer, "%s %s %s",
			_start_path,
		...
		if (dwCreationFlags == CREATE_SUSPENDED ||
			...
			dwCreationFlags == (CREATE_NO_WINDOW | CREATE_SUSPENDED))
		{
			dwCreationFlags = 0;
		}

		return HookDllData.dCreateProcessA(
			NULL,               // 传0 使用命令行
            buffer,             // 传给它调试目标
			lpProcessAttributes,
			lpThreadAttributes,
...
HANDLE WINAPI HookedCreateProcessW(LPCWSTR lpApplicationName, LPWSTR lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, DWORD dwCreationFlags, LPVOID lpEnvironment, LPCWSTR lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation)
{
	if (HookDllData.dCreateProcessW)
	{
		HoookFuncinit();

		LPCSTR subKey = SUB_KEY;
		LPSTR _start_pathA = GetRegDefaultValueString(subKey);   // 返回 ANSI 路径
		_start_pathA = StrRightTrim(_start_pathA, 6);
		_start_pathA = StrLeftTrim(_start_pathA, 1);

		// 转换成 Unicode
		WCHAR _start_pathW[MAX_PATH] = { 0 };
        N_MultiByteToWideChar(CP_ACP, 0, _start_pathA, -1, _start_pathW, MAX_PATH);

		WCHAR buffer[1024] = { 0 };
        N_wsprintfW(buffer, L"%s %s %s",
			_start_pathW,
		...
		return HookDllData.dCreateProcessW(
			NULL,               // 强制走 command line
			buffer,             // 完整命令行
			lpProcessAttributes,
			lpThreadAttributes,
			bInheritHandles,
